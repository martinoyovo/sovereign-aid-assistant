...
I recently demoed the [**Sovereign Aid Assistant**](https://github.com/martinoyovo/sovereign-aid-assistant) at [Open Source Week Community Day](https://www.unopensource.org/agenda), on the AI track. It's a fully offline AI assistant for NGO case data, and the live demo was the easy part to sell: a caseworker types a question in plain language, watches the agent search and reason over local files in real time, and gets an answer with every step traceable back to its source. The harder part to sell, and the actual point of the project, is what's *not* happening: nothing ever leaves the machine.
...
`callTool()` unwraps MCP's `content[]` array, JSON-parses the payload, and also generates a short human-readable summary ("8 records", "wrote brief", "error") used by the UI's live tool-call strip. That summary generation lives in the client adapter, not the tool layer, which keeps the MCP tool definitions themselves UI-agnostic.
...
`server/src/ollama.ts` is a plain HTTP client against `localhost:11434`, no SDK needed. `chat()` calls `/api/chat` with `stream: false`, which is a deliberate choice: the agent loop needs to inspect `tool_calls` as a complete structured object, not parse them out of partial streaming chunks. Streaming is pushed up to the SSE layer instead, at whole-turn (per agent step) granularity rather than token granularity, which simplifies both the agent loop and the frontend state machine considerably. Temperature is pinned to `0` for repeatable tool selection. A `warmModel()` call fires a 1-token completion at startup so the first real question isn't paying cold-load latency.
...
Each agent event is wrapped in a `{kind: "event", evt}` envelope, with a final `{kind: "done", result}` frame closing the stream (or `{kind: "error", ...}` on failure). That two-level envelope, a transport `kind` wrapping the agent's own event union, keeps SSE-specific concerns out of the agent loop's vocabulary.
...
|-------|:---:|-------|
| `qwen2.5` (~4.7 GB) | yes | Recommended primary; auto-selected as default. |
| `gemma4` (~9.6 GB) | yes | Good model-swap partner, different vendor. |
| `llama3.1` (~4.7 GB) | yes | Sends numeric tool args as strings; the tool layer coerces them. |
| `gemma3` / `gemma2` | no | No `tools` chat template in Ollama, fails with `"does not support tools"`. |
